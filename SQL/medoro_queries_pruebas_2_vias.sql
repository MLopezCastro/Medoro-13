...
/* PASO 4 — Función parametrizada (base común de la vía)
   - Ventana por días
   - Corrección -2 días
   - Horas por estado
   - Aliases: CodProducto, Motivo
...
(
    WITH DatosParseados AS (
        ...
        FROM dbo.ConCubo
        WHERE TRY_CONVERT(DATETIME, Inicio) >= DATEADD(DAY, -@DiasVentana, CONVERT(date, GETDATE()))
...
/* -------------------------------------------------------------
   SP: RefrescarV5_Incremental
   Vía: V5 (ConCuboSecuenciasBloques_Rango_M11_Completo_MAT)
   Estrategia:
     1) Corte por fecha = hoy - @DiasVentana
     2) DELETE de MAT donde FechaSecuencia >= corte
     3) Recalcular SOLO ese rango (V1→V2→V3→V4)
     4) Construir V5 (OrdenGlobalText + SecuenciaGlobalSQL)
        - SecuenciaGlobalSQL = ROW_NUMBER + @Offset
          (@Offset = MAX secuencia previa antes del corte)
   Supuestos:
     - No llegan datos con fecha anterior al corte. Si llegaran,
       habrá que forzar rebuild total para preservar la secuencia.
   ------------------------------------------------------------- */
...
BEGIN
    ...
    BEGIN TRY
        DECLARE @FechaCorte DATE = DATEADD(DAY, -@DiasVentana, CAST(GETDATE() AS DATE));
        ...
        PRINT 'V5 refrescada (últimos ' + CAST(@DiasVentana AS VARCHAR(10)) 
              + ' días) - ' + CONVERT(VARCHAR(20), GETDATE(), 120);
    END TRY
    BEGIN CATCH
...
BEGIN
  SET NOCOUNT ON;
  BEGIN TRY
    DECLARE @FechaCorte DATE = DATEADD(DAY, -@DiasVentana, CONVERT(date, GETDATE()));
    ...
    ,S AS (
      SELECT  -- mismo shape que la vista por eventos
        v.Renglon, v.ID,
    ...
    ,NEW_base AS (
      SELECT COALESCE(TRY_CAST(NroGlobal AS INT),
                      TRY_CAST(SUBSTRING(OP, PATINDEX('%[0-9]%', OP), 50) AS INT)) AS ID_Limpio,
             OP, CodAlfa, CodMaq,
             TRY_CAST(Alto AS INT) AS Alto, TRY_CAST(Ancho AS INT) AS Ancho,
             TRY_CAST(AltoV AS INT) AS AltoV, TRY_CAST(Fuelle AS INT) AS Fuelle
      FROM dbo.TablaVinculadaNEW
    )
    ,NEWmap AS (
      SELECT ID_Limpio,
    ...
    PRINT 'MED_V5E_Eventos_MAT refrescada (últimos ' + CAST(@DiasVentana AS VARCHAR(10)) + ' días).';
  END TRY
  BEGIN CATCH
